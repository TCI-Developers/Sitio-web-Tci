...
const Form = () => {
  return (
    <section className="md:py-12 md:flex md:justify-center tracking-normal">
      <div className="bg-white text-black px-4 py-8 rounded-3xl md:w-11/12 md:max-w-[400px]">
        <h2 className="font-bebas font-bold text-3xl sm:text-xl whitespace-normal leading-10 text-primary md:text-2xl tracking-tight">HAZ TU PREGUNTA</h2>
        <p className="font-wix leading-none mt-2 md:text-xs text-[#0F245F]">
          Si tienes una pregunta, puedes contactarnos.<br></br>
        ...
        <form className="p-2" action="https://formsubmit.co/alexisgmez65@gmail.com" method="POST">
          ...
          <div className="flex flex-col justify-start gap-8">
              <textarea className="border-b-2 border-b-black pb-8 focus:outline-none placeholder-gray-600" name="mensaje" placeholder="Mensaje" required />
              <button type="submit" className="bg-primary w-1/2 text-white p-4 rounded-2xl flex justify-between items-center font-wix">Enviar <FaArrowRight size={25} /></button>
          </div>
        </form>
      </div>
